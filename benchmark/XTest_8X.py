...
parser.add_argument('--path', default='/data/datasets/X4K1000FPS/test',type=str, required=False)
...
for strMode in ['XTEST-2k', 'XTEST-4k']:
    ...
    for intFrame in tqdm(listFiles):
        ...
        timestep = torch.tensor(intFrame[3]).reshape(1 , 1, 1, 1)

        padder = InputPadder(tenOne.shape, 32)
        tenOne, tenTwo = padder.pad(tenOne, tenTwo)
        tenEstimate = model.hr_inference(tenOne, tenTwo, TTA = TTA, timestep=timestep, down_scale = down_scale)[0]
