...
parser.add_argument('--path', default='/data/datasets/Xiph/test_4k',type=str, required=False)
...
for strCategory in ['resized', 'cropped']:
    ...
    for strFile in ['BoxingPractice', 'Crosswalk', 'DrivingPOV', 'FoodMarket', 'FoodMarket2', 'RitualDance', 'SquareAndTimelapse', 'Tango']: 
        for intFrame in range(2, 99, 2):
            ...
            npyEstimate = padder.unpad(model.hr_inference(tenFirst, tenSecond, timestep=timestep, TTA=TTA, down_scale=down_scale, fast_TTA=False).clamp(0.0, 1.0).cpu())[0]
