...
def calc_flolpips(dis_path, ref_path):
    ...
    for iFrame in range(num_frames-1):
        frame_dis = TF.to_tensor(read_frame_yuv2rgb(stream_dis, width, height, iFrame, 8, '420'))
        frame_dis_next = TF.to_tensor(read_frame_yuv2rgb(stream_dis, width, height, iFrame+1, 8, '420'))
        frame_ref = TF.to_tensor(read_frame_yuv2rgb(stream_ref, width, height, iFrame, 8, '420'))
        frame_ref_next = TF.to_tensor(read_frame_yuv2rgb(stream_ref, width, height, iFrame+1, 8, '420'))
