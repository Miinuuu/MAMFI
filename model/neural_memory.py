...
from .associative_scan import AssocScan
...
class NeuralMemory_bi(Module):
    def __init__(
        ...
        if not exists(model):
            ...
            model2 = MemoryMLP(dim_head, **default_model_kwargs)

        # validate memory model

        assert not exists(next(model.buffers(), None)), 'model cannot have buffers for now'

        test_shape = (2, 2, dim_head)
        ...
        self.memory_model2_parameters = ParameterList(memory_model2_parameters)
        ...
        def forward_and_loss1( params, inputs, loss_weights, target):
        ...
        def forward_and_loss2( params, inputs, loss_weights, target):

            pred = functional_call(self.memory_model2, params,inputs)
            loss = self.store_memory_loss_fn(pred, target) # simple mse loss in paper - eq (12) - |M(k) - v|²
            weighted_loss = loss * loss_weights
            return weighted_loss.sum(), loss
      
        # def forward_and_loss(params1,params2, inputs1,inputs2, loss_weights1,loss_weights2, target1,target2):

        #     pred = torch.cat([functional_call(self.memory_model,  params1,inputs1)
        #                      ,functional_call(self.memory_model2, params2,inputs2)])
        #     loss = self.store_memory_loss_fn(pred, torch.cat([target1,target2])) # simple mse loss in paper - eq (12) - |M(k) - v|²
        #     weighted_loss = loss * torch.cat([loss_weights1,loss_weights2])
        #     return weighted_loss.sum(), loss

        # two functions

        grad_fn1 = grad(forward_and_loss1, has_aux = True)
        grad_fn2 = grad(forward_and_loss2, has_aux = True)

        self.per_sample_grad_fn1 = vmap(grad_fn1, in_dims = (0, 0,0, 0))
        self.per_sample_grad_fn2 = vmap(grad_fn2, in_dims = (0, 0,0, 0))
    ...
    @property
    def memory_model2_parameter_dict(self):
        return TensorDict(dict(zip(self.memory_model2_parameter_names, self.memory_model2_parameters)))
    ...
    def init_weights(
        ...
        if self.per_head_learned_parameters:
            weights = repeat_dict_values(self.memory_model_parameter_dict, 'h ... -> (b h) ...', b = batch//2)
            weights2 = repeat_dict_values(self.memory_model2_parameter_dict, 'h ... -> (b h) ...', b = batch//2)
        else:
            weights = repeat_dict_values(self.memory_model_parameter_dict, '... -> bh ...', bh = batch//2 * self.heads)
            weights2 = repeat_dict_values(self.memory_model2_parameter_dict, '... -> bh ...', bh = batch//2 * self.heads)

        merged_weights = {
            key: torch.cat([weights[key], weights2[key]], dim=0)
    ...
    def init_momentum(
        ...
        zeros2 = self.memory_model2_parameter_dict.clone().zero_()

        if self.per_head_learned_parameters:
            zeros = repeat_dict_values(zeros, 'h ... -> o (b h) ...', b = batch//2, o = self.momentum_order)
            zeros2 = repeat_dict_values(zeros2, 'h ... -> o (b h) ...', b = batch//2, o = self.momentum_order)
        else:
            zeros = repeat_dict_values(zeros, '... -> o bh ...', bh = batch//2 * self.heads, o = self.momentum_order)
            zeros2 = repeat_dict_values(zeros2, '... -> o bh ...', bh = batch//2 * self.heads, o = self.momentum_order)

        merged_weights = {
            key: torch.cat([zeros[key], zeros2[key]], dim=1)
    ...
    def store_memories(
        ...
        if self.qkv_receives_diff_views: #true 
        ...
        heads, chunk_size, num_updates = self.heads, self.store_chunk_size, self.num_kv_per_token #1
        ...
        if not exists(weights):
            weights= self.init_weights(batch) # 양방향 고려해서 배치가 2개니까... batch * head(head별), hdim, hdim

        weights = TensorDict(weights)
        # weights2 = TensorDict(weights2)
        ...
        # weights_for_surprise2 = repeat_dict_values(weights2, 'b ... -> b n ...', n = num_chunks)
        # n = batch size /chunck, n = 256/4 = 64
        #batch * head * n , hdim, hdim

        #num_chunks = batch_szie/chunck  
        # initial norm  ,  batch * head ,  batchsize/chunck ,hdim,hdim

        seq = self.store_norm(seq)
        # batch, head, bathsize ,dim
        ...
        #batch,   batchsize/chuncksize, dim  - attention pool
        decay_factor = self.to_decay_factor(chunked_seq).sigmoid()
        #batch *head , batchsize/chuncksize, 1
        #suprise decay factor # sigmoid 0-1
        need_layer_lr_mod = exists(self.to_layer_modulation) and num_chunks > 0
        #self.to_layer_modulation ,  dim, head* depth mem
        ...
        if need_layer_lr_mod:
            ...
            # layer_lr_mod= mem depth, batch*head, batchsize/chuncksize
            # nn.Linear(dim, heads * self.num_memory_parameter_tensors),
            # Rearrange('b n (h w) -> w (b h) n', h = heads),
        ...
        if exists(prev_weights):
            ...
            #weights_for_surprise2 = weights_for_surprise2 + prev_weights
        ...
        weights_for_surprise1 = {}
        weights_for_surprise2 = {}

        for key in weights_for_surprise.keys():
            # 텐서를 2개의 청크로 분할합니다.
            weights_for_surprise1[key],  weights_for_surprise2[key] = torch.chunk(weights_for_surprise[key], chunks=2, dim=0)
        #weights_for_surprise2 = rearrange_dict_values(weights_for_surprise2, 'b n ... -> (b n) ...')
        # batch * head * n , hdim, hdim //b*h 인 이유는 퍼헤드 모멘텀?

        # get grads and extra auxiliary loss (for backwarding through qkv projection in base neural memory module)
        # if cross:
        #     values=torch.cat([values[batch//2:],values[:batch//2]],0)

        b=keys.shape[0]
        grads1, unweighted_mem_model_loss1 = self.per_sample_grad_fn1(dict(weights_for_surprise1), keys[:b//2], adaptive_lr[:b//2],values[b//2:])
        grads2, unweighted_mem_model_loss2 = self.per_sample_grad_fn2(dict(weights_for_surprise2), keys[b//2:], adaptive_lr[b//2:],values[:b//2])
        #        def forward_and_loss(params, inputs, loss_weights, target):

        grads = {
            key: torch.cat([grads1[key], grads2[key]], dim=0)
            for key in grads1.keys()
        }
        unweighted_mem_model_loss=torch.cat([unweighted_mem_model_loss1,unweighted_mem_model_loss2])

        # 프레임간의 변화율을 반영하는 방법을 적용해야한다. 
        # 인터리빙? or cross 
        # keys=  (b h n)  c  d  , b= batch ,  h = head, n = batch_size / chunck , c= chunck, d= dim 
        ...
        for (param_name, surprise), (_, last_update) in zip(surprises.items(), past_last_update.items()):
            ...
            if has_momentum:
                ...
                for one_adaptive_momentum, one_last_momentum in zip_longest(adaptive_momentum, last_momentum):
                    ...
                    momentum = self.assoc_scan(gates=one_adaptive_momentum, 
                                               inputs= momentum, 
                                               prev = one_last_momentum) # momentum is S / surprise in the paper
            ...
            update = self.assoc_scan(gates=(1. - decay_factor),
                                     inputs= update, #surprise
                                     prev = last_update, 
                                     remove_prev = False)
    ...
    def retrieve_memories(
        ...
        for key in weights.keys():
            # 텐서를 2개의 청크로 분할합니다.
            weights1[key],  weights2[key] = torch.chunk(weights[key], chunks=2, dim=0)


        b=queries.shape[0]
        values1 = functional_call(self.memory_model, dict(weights1), queries[:b//2])
        values2 = functional_call(self.memory_model2, dict(weights2), queries[b//2:])
    ...
    def forward(
        ...
        if is_multi_input:
            retrieve_seq, seq = seq[0], seq[1:] #q,k,v
        else:
            retrieve_seq = seq
